...
def process_course(
    ...
    VideoOrchestratorService.run_phase1_transcribe(state["needs_transcription"], whisper_model, w1)

    # 3. Phase 2: Tag and Sort
    print_header(f"Phase 2: Intelligent Tagging & Renaming ({w2} workers)")
    renamed_files = VideoOrchestratorService.run_phase2_tag_and_sort(raw_files, llm_model, w2)
    ...
    # 5. Phase 4: Merge
    VideoOrchestratorService.run_phase4_merge(target_dir, slideshow_files, max_merge_hours)
