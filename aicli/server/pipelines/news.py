...
from aicli.server.services.news_reasoning_service import NewsReasoningService, STANDARD_TOPICS
...
def _classify_batch(batch_idx: int, batch: list[str], system_prompt: str, client, model_name: str) -> tuple[int, list[dict], Exception | None]:
    try:
...
def parse_news(file_path: Path, output: Path = None, batch_size: int = 10, month: str = "Not Specified", year: str = "Not Specified", workers: int = 4, show_prompt: bool = False):
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), BarColumn(), MofNCompleteColumn(), console=console) as progress:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(_classify_batch, idx, batch, system_prompt, client, config.model_name): (idx, batch) for idx, batch in enumerate(batches)}
            for future in as_completed(futures):
                idx, batch = futures[future]
                batch_idx, records, err = future.result()

                if err:
                    progress.console.print(f"[red]✖ Batch {batch_idx + 1} failed: {err}[/red]")
                    records = [{"topic": "Miscellaneous", "tags": ""} for _ in batch]
                elif len(records) != len(batch):
                    progress.console.print(f"[yellow]⚠ Batch {batch_idx + 1}: Expected {len(batch)} records, got {len(records)}.[/yellow]")
                    while len(records) < len(batch): records.append({"topic": "Miscellaneous", "tags": ""})
                    records = records[: len(batch)]
                else:
                    progress.console.print(f"[green]✔ Batch {batch_idx + 1}: {len(records)} items classified[/green]")

                for i, rec in enumerate(records):
                    if rec.get("topic") not in STANDARD_TOPICS: rec["topic"] = "Miscellaneous"
                    rec["month"] = month
                    rec["year"] = year
...
def dedupe(file_path: Path, output: Path = None, threshold: float = 0.80, workers: int = 10):
...
def process_news(json_path: Path, output: Path = None, workers: int = 4, threshold: float = 0.8, force_merge: bool = False, no_cache: bool = False):
    ...
    if output.exists():
        ...
        console.print(f"[green]✔ Loaded {len(existing)} existing records. Total pool: {len(records)} items.[/green]")
    ...
    client = OpenAI(base_url=config.lm_studio_base_url, api_key=config.lm_studio_api_key)
    unique_records = _run_ai_merging(clusters, records, client, workers, output, force_merge=force_merge)
...
def _json_to_records(data: list) -> list[dict]:
    ...
    for item in data:
        ...
        news_str = "\n".join([p for p in [f"**{title}**" if title else "", key_answer, details] if p])
        ...
        item_json = json.dumps({
            "source": source, "order": order, "news": news_str,
            "title": title, "topic": topic, "tags": str(item.get("tags", ""))
        })
...
def _run_ai_merging(clusters: list[list[int]], records: list[dict], client, workers: int, output: Path, force_merge: bool = False) -> list[dict]:
    unique_records = []
    ...
    def _do_merge(job):
        merged = NewsReasoningService.merge_duplicate_news(job.pop("_news_strings"), client, config.model_name)
        job["news"] = merged
        return job
    ...
    unique_records.sort(key=lambda x: (x.get("source_key", "").split("|")[0].split("-")[0].strip().lower(), 999999))
