...
def rename_image(
    image_path: Path = typer.Argument(
        ...
        file_okay=True,
        dir_okay=False,
        help="Path to the image you want to rename."
    ),
    ...
    """
    Uses LM Studio Vision to scan an image and intelligently rename it based on its content.
    """
    ...
    with Progress(
        ...
        try:
            ...
            print_error("Failed to communicate with LM Studio.", e)
            print_error("Ensure LM Studio Local Server is running on http://localhost:1234/v1")
            raise typer.Exit(code=1)
            
    if not suggested_name:
        print_error("AI did not return a valid name.")
        raise typer.Exit(code=1)
