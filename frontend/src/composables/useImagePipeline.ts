...
export function useImagePipeline() {
  ...
  const startImagePipeline = async (endpoint: string, config: any) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8765/api/image/${endpoint}`, {
        method: 'POST',
      ...
      eventSource = new EventSource('http://127.0.0.1:8765/api/image/stream')
      eventSource.onmessage = (e) => {
        ...
        if (data.type === 'status') {
          ...
          if (data.status === 'completed') {
            logs.value.push(`[SYSTEM] Image Pipeline execution completed successfully.`)
            pipelineRunning.value = false
          }
