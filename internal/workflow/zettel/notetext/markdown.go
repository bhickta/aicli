...
func ExtractLineRanges(content string, ranges []model.LineRange) string {
	lines := SplitLines(content)
	var parts []string
...
func RemoveLineRanges(content string, ranges []model.LineRange) string {
	lines := SplitLines(content)
...
func MergeLineRanges(ranges []model.LineRange) []model.LineRange {
	filtered := make([]model.LineRange, 0, len(ranges))
	...
	merged := make([]model.LineRange, 0, len(filtered))
...
func ApplyMergePlan(target string, plan model.MergePlan) string {
