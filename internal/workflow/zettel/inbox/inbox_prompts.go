...
func inboxMergeMessages(sourcePath string, sourceContent string, candidates []scoredCandidate, options Options, shorthandPrompt string) []provider.Message {
	...
	return []provider.Message{
		{Role: "system", Content: strings.Join([]string{
			...
			"Preserve the destination note's existing markdown hierarchy: keep existing nested sub-bullets under their same parent unless the parent itself is being merged with an equivalent parent.",
			"When adding related source facts, place them under the existing relevant parent bullet or section; do not flatten old sub-bullets into top-level bullets.",
			...
			"Example: `CDS-2023-II` is an exam reference, not a `CDR/CDS` concept label; use `CDR` only for Currency Deposit Ratio unless the note explicitly defines another concept.",
